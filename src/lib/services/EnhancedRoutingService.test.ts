...
describe('EnhancedRoutingService', () => {
  ...
  describe('calculateRoute', () => {
    const testWaypoints: Waypoint[] = [
      ...
      { id: '2', name: 'End', lat: 36.2048, lng: -115.2640, category: 'attraction' }
    ];

    it('should throw error for insufficient waypoints', async () => {
      const service = new EnhancedRoutingService();
      
      await expect(service.calculateRoute([])).rejects.toThrow('At least 2 waypoints are required for routing');
      await expect(service.calculateRoute([testWaypoints[0]])).rejects.toThrow('At least 2 waypoints are required for routing');
    });
    ...
    it('should attempt real API call and fallback to mock on failure', async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith('OpenRouteService API failed, falling back to mock:', expect.any(Error));
