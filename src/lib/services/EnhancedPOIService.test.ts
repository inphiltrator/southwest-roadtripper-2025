...
describe('EnhancedPOIService', () => {
	...
	describe('mapOverpassCategory', () => {
		it('should map national park tags correctly', () => {
			expect(poiService['mapOverpassCategory']({ boundary: 'national_park' })).toBe(
				'national_park'
			);
			expect(
				poiService['mapOverpassCategory']({ leisure: 'park', protection_title: 'National Park' })
			).toBe('national_park');
			expect(
				poiService['mapOverpassCategory']({
		...
		it('should map state park tags correctly', () => {
			expect(poiService['mapOverpassCategory']({ leisure: 'park', park_type: 'state_park' })).toBe(
				'state_park'
			);
			expect(
				poiService['mapOverpassCategory']({ leisure: 'park', name: 'Valley of Fire State Park' })
			).toBe('state_park');
			expect(
				poiService['mapOverpassCategory']({ boundary: 'protected_area', protect_class: '5' })
			).toBe('state_park');
		});
