import type { LatLng, POI, POIResponse, SouthwestState } from '../types';
...
export class POIService {
  ...
  private buildOverpassQuery(location: LatLng, radius: number, categories: POI['category'][]): string {
    ...
    const queryParts = categories.map(category => {
      switch (category) {
        case 'national_park': return `node["leisure"="park"]["protection_title"="National Park"]${around};`;
        case 'camping': return `node["tourism"="camp_site"]${around};`;
        case 'dining': return `node["amenity"="restaurant"]${around};`;
        case 'attraction': return `node["tourism"="attraction"]${around};`;
        case 'lodging': return `node["tourism"="hotel"]${around};`;
        case 'fuel': return `node["amenity"="fuel"]${around};`;
        default: return '';
      }
    }).join('');
   ...
   */
  async getTopRatedPOIsInState(state: SouthwestState, category: POI['category'], limit = 5): Promise<POI[]> {
