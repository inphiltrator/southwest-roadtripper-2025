		...
		if (browser && mapContainer) {
			try {
				...
				const waypoints = [
					...
					{ lat: 33.4734, lng: -111.891, name: 'Phoenix, AZ', icon: '🌵' }
				];
...
<GlassCard class="overflow-hidden {className}">
	<div class="relative">
		<!-- Map Container -->
		<div bind:this={mapContainer} {style} class="w-full"></div>
		...
		{#if !browser}
			...
			>
				<div class="text-center">
					...
					<p class="font-medium text-southwest-canyon">Loading Southwest Map...</p>
				</div>
			</div>
		{/if}
		...
		<div class="absolute top-4 right-4 flex flex-col gap-2">
			<div class="rounded-lg bg-white/80 px-3 py-2 text-sm backdrop-blur-sm">
				<div class="font-bold text-southwest-sunset">Southwest USA</div>
