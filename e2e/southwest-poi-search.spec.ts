...
test.describe('Southwest POI Discovery - MCP Automated Tests', () => {
	...
	test('Grand Canyon POI search returns ≥1 national park result', async ({ page }) => {
		// Navigate to Grand Canyon area coordinates: 36.0544, -112.1401
		...
		await expect(page.locator('text=Searching Southwest attractions')).not.toBeVisible({
			timeout: 10000
		});
	...
	test('Grand Canyon POI search shows multiple categories', async ({ page }) => {
		...
		// Also verify other category icons if present
		const attractionIcon = page.locator('span:text("📍")');
		const lodgingIcon = page.locator('span:text("🏨")');
		const attractionCount = await attractionIcon.count();
		const lodgingCount = await lodgingIcon.count();
		console.log(`Found ${attractionCount} attractions, ${lodgingCount} lodging options`);
